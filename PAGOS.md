...
**Validaciones de Recalcular:** ningún dato obligatorio puede faltar. Los campos Total Objetivo, Nº de Cuotas y Cotización **arrancan vacíos** (un campo precargado pasaría la validación sin que el usuario haya decidido nada). Se chequea en dos pasos:

**1 · Datos obligatorios** — se juntan **todos** los que falten y se muestran en **un solo modal**, no de a uno. El foco va al primero de la lista:

```
Faltan completar 3 datos para generar el plan:
...
Son: Fecha de inicialización, Fecha de finalización, Total Objetivo, Nº de Cuotas y Cotización del dólar.

**2 · Coherencia** — solo si no falta ningún dato, y con modal individual porque cada una necesita explicar el porqué:

| Validación | Mensaje |
|---|---|
| Fin **no anterior** al inicio | "La Fecha de finalización no puede ser anterior a la de inicialización" |
| Objetivo **mayor** que el Nº de cuotas | "El Total Objetivo debe ser mayor que el Nº de cuotas (N)" |
